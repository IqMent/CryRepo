...
//Constants
static const unsigned long long SHA384_INITIAL_HASH_CONSTANTS[8] = {
...
static void sha384_transform(SHA384_CTX *ctx, unsigned char data[]){
    unsigned long long a, b, c, d, e, f, g, h;
    unsigned long long t1, t2;
    unsigned long long m[80];
    for (size_t i = 0, j = 0; i < 16; i++, j+=8)
    ...
    for (size_t i = 0; i < 80; i++){
        t1 = h + SIGMA1(e) + CH(e, f, g) + SHA384_CONSTANTS[i] + m[i];
...
int sha384_init(SHA384_CTX *ctx) { // 1 - success, 0 - fail
    if (!ctx)
        return (0);
    ctx->datalen = 0;
    ctx->bitlen = 0;
    ctx->state[0] = SHA384_INITIAL_HASH_CONSTANTS[0];
    ctx->state[1] = SHA384_INITIAL_HASH_CONSTANTS[1];
    ctx->state[2] = SHA384_INITIAL_HASH_CONSTANTS[2];
    ctx->state[3] = SHA384_INITIAL_HASH_CONSTANTS[3];
    ctx->state[4] = SHA384_INITIAL_HASH_CONSTANTS[4];
    ctx->state[5] = SHA384_INITIAL_HASH_CONSTANTS[5];
    ctx->state[6] = SHA384_INITIAL_HASH_CONSTANTS[6];
    ctx->state[7] = SHA384_INITIAL_HASH_CONSTANTS[7];
    return (1);
}

int sha384_update(SHA384_CTX *ctx, const unsigned char *data, const size_t len){ // 1 - success, 0 - fail
    if (!ctx || !data || len <= 0)
        return (0);
    unsigned long long i = ctx->datalen;
    for (; i < len; i++){
...
int sha384_final(SHA384_CTX *ctx, unsigned char hash[]){ // 1 - success, 0 - fail
    if (!ctx || !hash)
        return (0);
    unsigned long long i = ctx->datalen;
    if (ctx->datalen < 112){
